...
private:
	...
	static string page, botton;
	static int volunme;

	static void RestoreData(string& page, string& botton, int&volunme, char *buf);
...
public:
	static void Server();
	static bool SendFile(wstring filePathtemp);
	static unsigned __stdcall reciveMessage(void* p);
	static unsigned __stdcall sendMessage(void* p);
	static unsigned __stdcall startTCPServer(void* p);
};


//接收手机发送的消息
unsigned __stdcall TCPServer::reciveMessage(void* p){
...
//发送信息给手机
unsigned __stdcall TCPServer::sendMessage(void* p){
...
BOOL TCPServer::WStringToString(const std::wstring &wstr, std::string &str)
 {    
...
//发送文件给手机
bool TCPServer::SendFile(wstring filePathtemp){//, string fileName
...
//void TCPServer()
unsigned __stdcall TCPServer::startTCPServer(void* p)
{
	HANDLE hThread[2];
	//初始化WSA
	WSADATA wsaData;
	...
	while (true){
		...
		hThread[0] = (HANDLE)_beginthreadex(NULL, 0, &reciveMessage, 0, 0, NULL);
		hThread[1] = (HANDLE)_beginthreadex(NULL, 0, &sendMessage, 0, 0, NULL);
	}
...
** 全局变量:无
**************************************************/
void TCPServer::RestoreData(string& page, string& botton, int&volunme, char *buf){
	string str = buf;
